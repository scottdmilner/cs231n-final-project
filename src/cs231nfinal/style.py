...
class InjectionStylizer(Stylizer):
    def __init__(self, resolution: tuple[int, int], image_path: str, invert: bool = False) -> None:
        ...
        # global feat_maps
        self.feat_maps = [{
            'config': { 'gamma': self.gamma, 'T': self.T}
        } for _ in range(50)]
        ...
        with torch.no_grad():
            with self.precision_scope("mps"):
                with self.model.ema_scope():
                    ...
                    adain_z_enc = cnt_z_enc
                    self.feat_maps = self.feat_merge(self.gamma, self.T, cnt_feat, self.sty_feat, start_step=self.start_step)
                    
                    # if opt.without_attn_injection:
                    #     feat_maps = None

                    # inference
                    samples_ddim, intermediates = self.sampler.sample(S=self.ddim_steps,
                                                    batch_size=B,
                                                    shape=self.shape,
                                                    verbose=False,
                    ...
                    style_mask = camera_views < (1.0 - 1e-6)
                    return x_image_torch * style_mask + (
                        1 - style_mask.to(torch.float32)
                    )
